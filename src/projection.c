...
int projection(obj_t *object, camera_t *camera, vector3_t *light, int *image)
{
    ...
    float z_buffer[camera->height * camera->width];         // need a more efficient way of doing this (maybe memcpy ?)
    ...
    if (view_to_clip == NULL)
        err(EXIT_FAILURE, "Error while allocating memory for view_to_clip matrix");
    tmatrix_init(view_to_clip);

    tmatrix_t *world_to_view = malloc(sizeof(struct tmatrix_t));
    if (world_to_view == NULL)
        err(EXIT_FAILURE, "Error while allocating memory for world_to_view matrix");
    tmatrix_init(world_to_view);

    tmatrix_t *projMatrix = malloc(sizeof(struct tmatrix_t));
    if (projMatrix == NULL)
        err(EXIT_FAILURE, "Error while allocating memory for projMatrix matrix");
    ...
    norm = fastisqrt(camera_ux.x * camera_ux.x + camera_ux.y * camera_ux.y + camera_ux.z * camera_ux.z);
    camera_ux.x = camera_ux.x * norm;
    camera_ux.y = camera_ux.y * norm;
    camera_ux.z = camera_ux.z * norm;

    /*
     * camera_look_at and camera_right are already normalized, no need to do it to camera_down
     */
    ...
    for (size_t face_i = 0; face_i < object->face_count; face_i++)
    {
        ...
        vector3_set(&AB,triangle[1]->x-triangle[0]->x,triangle[1]->y-triangle[0]->y,triangle[1]->z-triangle[0]->z);
        vector3_set(&AC,triangle[2]->x-triangle[0]->x,triangle[2]->y-triangle[0]->y,triangle[2]->z-triangle[0]->z);
        v_cross_v(&AB,&AC,&N);
        ...
        float culling ;
        vector3_set(&view,
                    triangle[0]->x-camera->position->x,
        ...
        //Raster stage
        build_triangle(projTriangle, projTriangle + 1, projTriangle + 2, (int) color,
                       image, z_buffer, camera->width, camera->height);
    }
