...
int     face_add_vertex(face_t *face, size_t v, size_t vt, size_t vn) {
    ...
    if ((face->v_indices = reallocarray(face->v_indices, new_count, size)) == NULL)
        return OBJ_ERROR;        // realloc() error
    face->v_indices[face->vertex_count] = v;

    /* Store texture vertex index */
    if ((face->vt_indices = reallocarray(face->vt_indices, new_count, size)) == NULL)
        return OBJ_ERROR;        // realloc() error
    face->vt_indices[face->vertex_count] = vt;

    /* Store normal vertex index */
    if ((face->vn_indices = reallocarray(face->vn_indices, new_count, size)) == NULL)
        return OBJ_ERROR;        // realloc() error
...
int     obj_add_texture_coordinate(obj_t *obj, vector3_t *vt) {
    ...
    if ((obj->texture_coords = reallocarray(obj->texture_coords, new_count, size)) == NULL)
        return OBJ_ERROR;
